...
- If a single Claim needs to be attested by multiple issuers, then an ACL must be created by each issuer for that Claim. We call the group of ATs that have the same Claim a "composite ACL".
...
- It is association of an ACL with a POA created by the issuer of that ACL; i.e., an ATT is a signed ACL. 
...
- In the case of a "composite ACL", an ATT must be created for each ACL by its issuer. Similarly, we call the collection of those ATs a "composite ATT".
...
An example CLT of a property sales agreement (an example of `nested ATs`):
```json
{
